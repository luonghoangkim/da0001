...
const LoginForm = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    if (!isValidEmail(email)) {
      ...
      toast.error(t('invalidEmail')); // Hiển thị thông báo lỗi
      return;
    }

    if (!password) {
      setError(t('invalidPassword'));
      toast.error(t('invalidPassword')); // Hiển thị thông báo lỗi
      return;
    }
    ...
    try {
      ...
      if (res.status === 400) {
        ...
        toast.error(t('emailExists')); // Hiển thị thông báo lỗi
      } else if (res.status === 200) {
        setError('');
        toast.success(t('loginSuccess')); // Hiển thị thông báo thành công
        router.push('/dashboard');
      }
    } catch (error) {
      setError(t('errorOccurred'));
      toast.error(t('errorOccurred')); // Hiển thị thông báo lỗi
      console.log(error);
    }
  };
