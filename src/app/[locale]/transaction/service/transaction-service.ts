...
export const createTransaction = async (payload: { amount: number, description: string, category_id: string, type: string, status: string }) => {
...
export const updateTransaction = async (card_id: string, bank_name: string, card_number: string) => {
    const token = localStorage.getItem('authToken');
    try {
        const res = await fetch(`${API_URL}/api/transaction`, {
            method: 'PATCH',
